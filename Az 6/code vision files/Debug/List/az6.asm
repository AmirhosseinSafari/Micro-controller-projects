...
;NAME DEFINITIONS FOR GLOBAL VARIABLES ALLOCATED TO REGISTERS
	...
	.DEF _temp_sensor_input=R6
	.DEF _temp_sensor_input_msb=R7
	.DEF __lcd_x=R9
	.DEF __lcd_y=R8
	.DEF __lcd_maxx=R11
...
_0x0:
	.DB  0x70,0x6F,0x74,0x20,0x3D,0x20,0x0,0x20
	.DB  0x28,0x76,0x29,0x0,0x74,0x65,0x6D,0x70
	.DB  0x20,0x3D,0x20,0x0,0x20,0x28,0x43,0x29
	.DB  0x0
_0x2000060:
	.DB  0x1
_0x2000000:
...
;char pot_str[6];
;
;int temp_sensor_input;
;float temp_sensor_input_float;
;char temp_str[6];
;
...
; 0000 007C ASSR=0<<AS2;
	OUT  0x22,R30
; 0000 007D TCCR2=(0<<PWM2) | (0<<COM21) | (0<<COM20) | (0<<CTC2) | (0<<CS22) | (0<<CS21) | (0<<CS20);
	OUT  0x25,R30
...
; 0000 009B // Only the 8 most significant bits of
; 0000 009C // the AD conversion result are used
; 0000 009D ADMUX=ADC_VREF_TYPE;
	LDI  R30,LOW(224)
	OUT  0x7,R30
; 0000 009E ADCSRA=(1<<ADEN) | (0<<ADSC) | (1<<ADATE) | (0<<ADIF) | (0<<ADIE) | (0<<ADPS2) | (0<<ADPS1) | (1<<ADPS0);
	LDI  R30,LOW(161)
	OUT  0x6,R30
...
; 0000 00B9       pot_input = read_adc(0);
	LDI  R26,LOW(0)
	RCALL _read_adc
	MOV  R4,R30
	CLR  R5
; 0000 00BA       pot_input_float = pot_input;
	MOVW R30,R4
	LDI  R26,LOW(_pot_input_float)
	LDI  R27,HIGH(_pot_input_float)
	CALL SUBOPT_0x0
; 0000 00BB       pot_input_float = pot_input_float / 100;
...
; 0000 00BC       ftoa(pot_input_float, 2, pot_str);
	CALL __PUTPARD1
	LDI  R30,LOW(2)
	ST   -Y,R30
	...
	CALL _ftoa
; 0000 00BD 
; 0000 00BE       temp_sensor_input = read_adc(1);
	LDI  R26,LOW(1)
	RCALL _read_adc
	MOV  R6,R30
	CLR  R7
; 0000 00BF       temp_sensor_input_float = temp_sensor_input;
	MOVW R30,R6
	LDI  R26,LOW(_temp_sensor_input_float)
	LDI  R27,HIGH(_temp_sensor_input_float)
	CALL SUBOPT_0x0
; 0000 00C0       ftoa(temp_sensor_input_float, 2, temp_str);
	LDS  R30,_temp_sensor_input_float
	LDS  R31,_temp_sensor_input_float+1
	LDS  R22,_temp_sensor_input_float+2
	LDS  R23,_temp_sensor_input_float+3
	CALL __PUTPARD1
	LDI  R30,LOW(2)
	ST   -Y,R30
...
; 0000 00C2       lcd_gotoxy(0,0);
	...
	LDI  R26,LOW(0)
	CALL _lcd_gotoxy
; 0000 00C3       lcd_puts("pot = ");
	__POINTW2MN _0x9,0
	CALL _lcd_puts
; 0000 00C4       lcd_puts(pot_str);
	LDI  R26,LOW(_pot_str)
	LDI  R27,HIGH(_pot_str)
	CALL _lcd_puts
; 0000 00C5       lcd_puts(" (v)");
	__POINTW2MN _0x9,7
	CALL _lcd_puts
; 0000 00C6 
; 0000 00C7       lcd_gotoxy(0,1);
	...
	LDI  R26,LOW(1)
	CALL _lcd_gotoxy
; 0000 00C8       lcd_puts("temp = ");
	__POINTW2MN _0x9,12
	CALL _lcd_puts
; 0000 00C9       lcd_puts(temp_str);
	LDI  R26,LOW(_temp_str)
	LDI  R27,HIGH(_temp_str)
	CALL _lcd_puts
; 0000 00CA       lcd_puts(" (C)");
	__POINTW2MN _0x9,20
	CALL _lcd_puts
; 0000 00CB       }
	RJMP _0x6
